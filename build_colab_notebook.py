...
=======================
Generates OULAD_All_Models_Colab.ipynb — a self-contained Google Colab
notebook that trains and compares all models on the OULAD dataset.
...
def md(src):
    return cell(src, kind="markdown")

def code(src):
    return cell(src, kind="code")
...
"\n"
"**Models:** XGBoost · LightGBM · CatBoost · Random Forest · Extra Trees · "
"Bagged DT · Decision Tree · MLP · DNN (PyTorch GPU/CPU)\n\n"
"**Tasks:** Binary (AtRisk vs Success) and 4-Class (Distinction/Fail/Pass/Withdrawn)\n\n"
"> All features are **end-of-course** (final-outcome prediction). "
"For early-warning use the Week-8 table.\n"
"\n---"
))
...
# ── 2. Upload / Mount ───────────────────────────────────────────────────────
cells.append(md("## 2. Upload or mount the OULAD ML table\n\n"
"Set `USE_DRIVE = True` if `oulad_ml_table.csv` is in your Google Drive. "
"Otherwise the cell will prompt you to upload it directly."))
cells.append(code(
"import os\n"
"from pathlib import Path\n"
"\n"
...
# ── 3. Imports ──────────────────────────────────────────────────────────────
cells.append(md("## 3. Imports & global config"))
cells.append(code(
...
cells.append(md("## 4. OULAD feature engineering\n\n"
...
# ── 5. Load data ──────────────────────────────────────────────────────────
cells.append(md("## 5. Load and prepare data"))
cells.append(code(
...
# ── 6. Preprocessors ─────────────────────────────────────────────────────
cells.append(md("## 6. Preprocessing pipelines"))
cells.append(code(
...
# ── 7. DNN definition ────────────────────────────────────────────────────
cells.append(md("## 7. PyTorch DNN (GPU/CPU)"))
cells.append(code(
...
# ── 8. Model registry ────────────────────────────────────────────────────
cells.append(md("## 8. Model registry (all 9 models)"))
cells.append(code(
...
# ── 9. Evaluation helper ────────────────────────────────────────────────
cells.append(md("## 9. Evaluation helper"))
cells.append(code(
...
# ── 10. Training loop ──────────────────────────────────────────────────
cells.append(md("## 10. Training loop\n\nRuns all 9 models on binary **and** 4-class tasks. "
"Uses a single shared 80/20 stratified split per task so results are directly comparable."))
cells.append(code(
...
# ── 11. Summary table ──────────────────────────────────────────────────
cells.append(md("## 11. Summary results table"))
cells.append(code(
...
# ── 12. Performance bar chart ───────────────────────────────────────────
cells.append(md("## 12. Performance bar charts — Binary & 4-Class"))
cells.append(code(
...
# ── 13. ROC curves ────────────────────────────────────────────────────
cells.append(md("## 13. ROC curves — all models"))
cells.append(code(
...
cells.append(md("## 14. Confusion matrices — all models"))
cells.append(code(
...
# ── 15. Precision-Recall ──────────────────────────────────────────────
cells.append(md("## 15. Precision-Recall curves — binary"))
cells.append(code(
"fig, ax = plt.subplots(figsize=(9,7))\n"
"for mk in MODEL_ORDER:\n"
...
# ── 16. SHAP ──────────────────────────────────────────────────────────
cells.append(md("## 16. SHAP feature importance\n\n"
"SHAP TreeExplainer for tree-based models (fast). "
"KernelExplainer for MLP/DNN (slow — uses 100 background samples)."))
cells.append(code(
...
# ── 17. SHAP plots ────────────────────────────────────────────────────
cells.append(md("## 17. SHAP importance bar chart grid"))
cells.append(code(
...
# ── 18. Heatmap ───────────────────────────────────────────────────────
cells.append(md("## 18. Performance heatmap"))
cells.append(code(
...
# ── 19. Training time ──────────────────────────────────────────────────
cells.append(md("## 19. Training time comparison"))
cells.append(code(
...
# ── 20. Download results ───────────────────────────────────────────────
cells.append(md("## 20. Download results"))
cells.append(code(
"import zipfile\n"
...
out = Path(__file__).parent / "OULAD_All_Models_Colab.ipynb"
